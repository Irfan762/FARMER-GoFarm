...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_fruit'])) {
    ...
    // Prepare the SQL statement - FIXED: Removed 'unit' from the field list
    $stmt = $conn->prepare("INSERT INTO vegitable (name, price, quantity, image, info, videopath) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    // Bind parameters - FIXED: Removed 'unit' from the parameters
    $stmt->bind_param("sdssss", $name, $price, $quantity, $image, $info, $videopath);
