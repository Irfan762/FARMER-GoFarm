...
$result = $conn->query("SELECT COUNT(*) as count FROM productss");
...
if ($row['count'] == 0) {
    ...
    $stmt = $conn->prepare("INSERT INTO productss (name, price, category, image) VALUES (?, ?, ?, ?)");
...
function removeProduct($conn, $id) {
    $stmt = $conn->prepare("DELETE FROM productss WHERE id = ?");
